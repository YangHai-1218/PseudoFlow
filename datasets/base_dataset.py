...
import glob, json, random, time, tqdm
import mmcv
...
from .pose import project_3d_point, eval_rot_error, eval_tran_error
...
class BaseDataset(Dataset):
    ...
    def evaluate(self,
        ...
        Args:
            results (list[dict]): Testing results of the whole test dataset.
                Each entry is formated as following:
                    dict(
                        "gt": {'labels':[], 'translations':[], 'rotations':[], *'bboxes':[],},
                        "pred": {'labels':[], 'scores':[], 'translations':[], 'rotations':[], *'bboxes'[]},
                        *"mask_metric": {"intersect":[], "union":[], "pred":[], "gt":[]},
                        "img_metas": {},
                    ),
        ...
        '''
        supported_metric = ['add', 'rep', 'epe', 'epe_noc', 'rotation', 'translation', 'depth', 'trans_xy', 'auc', 'similarity', 'bop']
        metrics = metric.copy()
        for metric_name in metrics.keys():
            assert metric_name in supported_metric, f"{metric_name} is currently not supported"
        
        if 'bop' in metric:
            # for now, don't support calculating BOP and other metrics at the same time
            self.format_results(results, save_dir='work_dirs/temp_result')
            dataset_name = osp.basename(osp.dirname(self.data_root))
            if 'val' in self.data_root:
                bop_json = 'val_targets_bop19.json'
                tag = 'val'
            else:
                bop_json = 'test_targets_bop19.json'
                tag = 'test'
            bop_path = osp.join(osp.dirname(self.data_root), bop_json)
            print_log('\n Evaluating BOP Metric...')
            start = time.time()
            os.system(f'./eval_bop19.sh work_dirs/temp_result {bop_path} work_dirs/temp_result/PFA_{dataset_name}-{tag}.csv {bop_json} >work_dirs/temp_result/test.log 2>&1')
            end = time.time()
            print_log(f'\n Evaulation Done! It takes {round(end-start, 2)}s')
            score_path = osp.join("work_dirs/bop19_eval/mvc_subfolder", f"PFA_{dataset_name}-{tag}", 'scores_bop19.json')
            with open(score_path, 'r') as f:
                scores = json.load(f)
                score_ar = scores['bop19_average_recall']
                score_vsd = scores['bop19_average_recall_vsd']
                score_mssd = scores['bop19_average_recall_mssd']
                score_mspd = scores['bop19_average_recall_mspd']
            os.system('rm -rf work_dirs/temp_result')
            print_log('\n' + f'>>[BOP Metric] Average Recall:{score_ar}, Average Recall VSD:{score_vsd}, Average Recall MSSD:{score_mssd}, Average Recall MSPD:{score_mspd} <<')
            return dict(AR=score_ar, VSD=score_vsd, MSSD=score_mssd, MSPD=score_mspd)
        ...
        if 'add' in metric or 'rep' in metric:
            ...
            if 'auc' in metric:
                # cal auc, given the sample accuracy
                error_dict['auc'] = error_3d
            # np.save('debug/ycbv_add_transformed.npy', error_3d_normalized_)
            # add_depth(error_3d_, translation_gts[matched_gt_flag], translation_preds[matched_gt_flag], 5)
            # vis_add_dis(error_3d_)
            

        
        if 'translation' in metric or 'depth' in metric or 'trans_xy' in metric:
            error_trans = np.full_like(labels, dtype=np.float32, fill_value=20)
            error_depth = np.full_like(labels, dtype=np.float32, fill_value=20)
            error_xy = np.full_like(labels, dtype=np.float32, fill_value=20)
            error_trans_, error_depth_, error_xy_ = eval_tran_error(translation_gts[matched_gt_flag], translation_preds[matched_gt_flag])
            # np.savetxt('debug/error_depth_2.npy', error_depth_)
            error_trans_ = error_trans_ / 10.
            error_depth_ = error_depth_ / 10.
            error_xy_ = error_xy_ / 10.
            error_trans[matched_gt_flag] = error_trans_
            error_depth[matched_gt_flag] = error_depth_
            error_xy[matched_gt_flag] = error_xy_
            error_dict['translation'] = error_trans
            error_dict['depth'] = error_depth
            error_dict['trans_xy'] = error_xy
        
        if 'rotation' in metric:
            error_rot = np.full_like(labels, dtype=np.float32, fill_value=20)
            error_rot_ = eval_rot_error(rotation_gts[matched_gt_flag], rotation_preds[matched_gt_flag])
            error_rot[matched_gt_flag] = error_rot_
            error_dict['rotation'] = error_rot
        
        # if 'epe' in metric:
        #     concat_epe = np.concatenate([result['pred']['epe'] for result in results])
        #     concat_epe = concat_epe[global_matched_indexs]
        #     error_dict['epe'] = concat_epe
        #     concat_epe_noc = np.concatenate([result['pred']['epe_noc'] for result in results])
        #     concat_epe_noc = concat_epe_noc[global_matched_indexs]
        #     error_dict['epe_noc'] = concat_epe_noc
    
        # if 'similarity' in metric:
        #     concat_similarity = np.concatenate([result['pred']['similarity'] for result in results])
        #     concat_similarity = concat_similarity[global_matched_indexs]
        #     error_dict['similarity'] = concat_similarity
    ...
    def eval_auc_metric(self, error_3ds, max_error):
        ...
        for i in range(binCnt):
            validCnt = (error_3ds <= ((i+1) * (max_error/binCnt))).sum()
